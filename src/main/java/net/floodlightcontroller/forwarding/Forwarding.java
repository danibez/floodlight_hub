...
//import net.floodlightcontroller.hub.Hub.HubType;
...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener, ILinkDiscoveryListener,
	...
	private TransportPort initialPort;
...
//		        	setMaster(hostPorts, ipSrc);
		        	if((tcp.getDestinationPort().toString().compareTo("5001") == 0) || (tcp.getDestinationPort().toString().compareTo("5672") == 0))
		        	{
...
//		        		setMaster(hostPorts, ipDst);
		        		if((ipSrc.compareTo("10.10.0.1") == 0) || (ipSrc.compareTo("10.10.0.2") == 0)) {
		        			initialPort = tcp.getSourcePort();
//		        			log.info("{}", initialPort.toString());
...
//			        		hostPorts.add(OFPort.of(5));
		        			...
		        			packetOutMultiPort(pi, sw, OFPort.of(p), 0, hostPorts, initialPort, cntx);
...
//			        		pushPacket(sw, pi, OFPort.of(5), true, cntx);
	...
	//		        		doL2ForwardFlow(sw, pi, decision, cntx, false);
		        	...
		        	else if((tcp.getSourcePort().toString().compareTo("5001") == 0) || (tcp.getSourcePort().toString().compareTo("5672") == 0)) {
		        		ipSrc = ipv4.getSourceAddress().toString();
		        		String ipDst = ipv4.getDestinationAddress().toString();
...
//		        			log.info("pin:{}, currentMaster:{}", pin, currentMaster);
//		        			log.info("{}",tcp.getDestinationPort().toString());
		        			if(currentMaster == 0)
		        				currentMaster = pin;
		        			if (!isHostAlive())
		        				currentMaster = pin;
		        			if(currentMaster == pin) {
...
//		        				log.info("master: {}, ip: {}", masterPort, ipSrc);
		        				packetOutMultiPort(pi, sw, OFPort.of(pin), currentMaster, outPort, initialPort,cntx);
//		        				doL2ForwardFlow(sw, pi, decision, cntx, false);
//		        				log.info("master: {}, ip: {}", masterPort, ipSrc);
